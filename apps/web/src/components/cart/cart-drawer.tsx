          ...
          {rows.length === 0 ? (
            ...
            <>
              ...
              {availableUpsells.length > 0 && (
                <div className="border-t border-zinc-200 bg-zinc-50/60 p-4 dark:border-white/10 dark:bg-zinc-900/20">
                  <div className="mb-3">
                    <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                      Recommended add-ons
                    </h3>
                    <p className="text-xs text-zinc-600 dark:text-zinc-400">
                      A few bottles that pair well with what you picked.
                    </p>
                  </div>
...
function UpsellItem({ product, onAdd }: { product: Product; onAdd: () => void }) {
  ...
  return (
    <div className="flex min-w-[160px] flex-col gap-2 rounded-xl border border-zinc-200 bg-white p-3 shadow-sm dark:border-white/10 dark:bg-black">
      <Link
        href={`/shop/${product.slug}`}
        className="relative aspect-square w-full overflow-hidden rounded-lg bg-zinc-100 dark:bg-zinc-800"
      >
        {product.imageUrl ? (
          <AppImage src={product.imageUrl} alt={product.name} fill sizes="160px" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-2xl">
            🍷
          </div>
